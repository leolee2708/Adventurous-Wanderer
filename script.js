...
function setCost(cost) {
    ...
    myCost = cost;
    console.log(myCost);
    userChoices.cost = myCost;
}
var myActivites = null;
function setActivites(Acts){
    myActivites = Acts;
    console.log(myActivites);
//  check before  if (userChoices.activities.length =< 2 choices
    userChoices.activities.push(myActivites);
    console.log(userChoices.activities);
    // check after
}

function filterUserchoices() {
    var filterRegion = allDestinations
    if (userChoices.region != "") {
        filterRegion = filterRegion.filter(destination => 
            destination.region = userChoices.region)
    }
    console.log(filterRegion);
    if (userChoices.cost != ""){
        filterCost = filterCost.filter(destination =>
            destination.cost = userChoices.cost)
    }
    console.log(filterCost)
    //do the rest for climates and for activities
    // if (userChoices. != "")
    
}


//implement it to other button
// Array of all the available destinations
// var allDestinations = [
//     {
//         "name": "ireland",
//         "region": "land", // filtered based on this property when a user clicks one of those 4 region buttons
//         "latitude": "12.23",
//         "longitude": "-21.33",
//         "budget": "medium",
//         "isCold": true,
...
//     {
//         "name": "hawaii",
//         "region": "island", // filtered based on this property when a user clicks one of those 4 region buttons
//         "latitude": "233.23",
//         "longitude": "-234.33",
//         "budget": "high",
//         "isCold": false,
...
// ex: user clicks on land button

// its going to call the myRegion function
// var myRegion = null;
// function setRegion(region) {
//     myRegion = region;
...
//         return destination.budget = budget;
//     });
// }
// apply other filters
// var climate =
// var activities =
// lets say after all the filters are applied, you are left with 3 destinatinos (array of 3) for userInterestedDestinations
...
// var randomIndex = Math.Random(0, numberOfRemainingDestinations);
// grab the coordinates
// userInterestedDestinations[randomIndex].latitude
// userInterestedDestinations[randomIndex].longitude
// feed this into your map
